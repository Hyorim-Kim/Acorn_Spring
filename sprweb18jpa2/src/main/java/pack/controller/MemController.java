...
public class MemController {
	...
	public String insert(MemBean bean, Model model) {
		String msg = dataProcess.insert(bean);  // client가 입력한 bean 값 들어감
		
		if(msg.equals("success")) {
			//return "list";  // forwarding, 바로 list 요청이 들어감
			return "redirect:http://localhost/list";  // redirect 방식, 클라이언트를 통해 요청하기 때문에 19번줄을 만남
		}else {
	...
	public String delete(@RequestParam("num")int num, Model model) {
		...
		if(msg.equals("success")) {
			//return "list";  // forwarding, 바로 list 요청이 들어감
			return "redirect:http:list";  // redirect 방식, 클라이언트를 통해 요청하기 때문에 19번줄을 만남
		}else {
			model.addAttribute("msg", msg);
			return "error.html";
		}
